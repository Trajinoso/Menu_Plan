...
import { Recipe, MealType, getRecipeCategories } from '../types';
...
export const RecipesView: React.FC<RecipesViewProps> = ({
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'list'>(() => {
    try {
      const saved = localStorage.getItem('mm_recipes_view_mode');
  ...
  const toggleViewMode = (mode: 'grid' | 'list') => {
    setViewMode(mode);
    try {
      localStorage.setItem('mm_recipes_view_mode', mode);
    } catch (e) {}
  };
  ...
  // For the Assign to Plan modal (starts empty, no day preselected)
  ...
  const handleOpenAssignModal = (recipe: Recipe) => {
    isAssigningRef.current = false;
    setAssigningRecipe(recipe);
    setSelectedDays([]); // Ningún día marcado por defecto
  ...
  const filters = React.useMemo(() => {
    ...
    recipes.forEach((r) => {
      getRecipeCategories(r).forEach((c) => { if (c.trim()) set.add(c.trim()); });
    });
  ...
  const filteredRecipes = recipes.filter((recipe) => {
    ...
    const recipeCats = getRecipeCategories(recipe).map((c) => c.toLowerCase());
    const matchCat = recipeCats.includes(target);
    const matchTag = recipe.tags?.some((t) => t.toLowerCase() === target);
  ...
  const handleConfirmAssign = () => {
    if (isAssigningRef.current || !assigningRecipe || selectedDays.length === 0) return;
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-5 rounded-2xl border border-[#e1e3e4] shadow-xs">
        ...
        <div className="flex flex-wrap items-center gap-2.5 sm:gap-3 w-full md:w-auto">
          ...
          {/* Toggle Vista: Cuadrícula / Listado */}
          <div className="flex items-center bg-[#f3f4f5] p-1 rounded-xl border border-[#e1e3e4] shrink-0">
            <button
              type="button"
            ...
            <button
              ...
              title="Vista en listado rápido (solo nombre y categoría)"
              className={`px-2.5 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer ${
                viewMode === 'list'
                  ? 'bg-white text-[#0f5238] shadow-xs'
                  : 'text-[#707973] hover:text-[#191c1d]'
          ...
          <button
            ...
            className="shrink-0 bg-[#0f5238] hover:bg-[#2d6a4f] text-white text-xs md:text-sm font-semibold px-3 sm:px-4 py-2 rounded-xl transition-all shadow-xs flex items-center gap-1.5 cursor-pointer active:scale-98"
          >
            <Plus className="w-4 h-4 stroke-[2.5]" />
            <span className="hidden sm:inline">Añadir Receta</span>
      ...
      {/* Recipes Content: Grid or List */}
      {filteredRecipes.length > 0 ? (
        viewMode === 'list' ? (
          /* List View: Solo nombre de la receta y categoría para navegación ultra rápida */
          <div className="bg-white rounded-2xl border border-[#e1e3e4] overflow-hidden shadow-xs divide-y divide-[#e1e3e4]">
            {filteredRecipes.map((recipe) => (
              <div
                key={recipe.id}
                className="flex items-center justify-between gap-3 p-2.5 sm:px-4 hover:bg-[#f8f9fa] transition-colors group"
              >
                {/* Nombre de la receta y Categorías */}
                <div
                  onClick={() => setSelectedDetailRecipe(recipe)}
                  className="flex items-center gap-3 min-w-0 flex-1 cursor-pointer"
                  title="Haz clic para ver detalles de la receta"
                >
                  <div className="min-w-0 flex-1 flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 sm:flex-wrap">
                    <h3 className="font-bold text-sm sm:text-base text-[#191c1d] group-hover:text-[#0f5238] truncate transition-colors">
                      {recipe.name}
                    </h3>
                    <div className="flex flex-wrap items-center gap-1">
                      {getRecipeCategories(recipe).map((cat) => (
                        <span
                          key={cat}
                          className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-semibold bg-[#0f5238]/10 text-[#0f5238] border border-[#0f5238]/20 shrink-0"
                        >
                          {cat}
                        </span>
                      ))}
                    ...
                    {recipe.isAiGenerated && (
                      <span className="inline-flex self-start sm:self-auto items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-[#fc8a40]/15 text-[#fc8a40] shrink-0">
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  <button
                    type="button"
                    onClick={() => handleOpenAssignModal(recipe)}
                    className="bg-[#f8f9fa] hover:bg-[#0f5238] text-[#0f5238] hover:text-white border border-[#bfc9c1] hover:border-[#0f5238] font-semibold text-xs py-1.5 px-2.5 rounded-lg transition-all flex items-center gap-1 cursor-pointer active:scale-98"
                    title="Añadir al plan semanal"
                  >
                    <Plus className="w-3.5 h-3.5 stroke-[2.5]" />
                    <span className="hidden sm:inline">Añadir al plan</span>
                    <span className="sm:hidden">Añadir</span>
                  </button>
                  ...
                  {onEditRecipe && (
                    <button
                      ...
                      title="Editar receta"
                      className="p-1.5 text-[#0f5238] hover:bg-[#b1f0ce]/40 border border-[#bfc9c1] hover:border-[#0f5238] rounded-lg text-xs font-semibold flex items-center transition-all cursor-pointer"
                    >
                  ...
                  {onDeleteRecipe && (
                    confirmDeleteId === recipe.id ? (
                      <button
                        ...
                        className="px-2 py-1.5 bg-[#ba1a1a] hover:bg-[#93000a] text-white rounded-lg text-xs font-semibold flex items-center gap-1 transition-all cursor-pointer shrink-0 animate-in fade-in"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 text-[#707973] hover:text-[#ba1a1a] hover:bg-[#ffdad6]/40 border border-transparent hover:border-[#ba1a1a]/30 rounded-lg transition-all cursor-pointer shrink-0"
                      >
          ...
          /* Grid View: Cuadrícula con fotos compactas */
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
          {filteredRecipes.map((recipe) => (
            <div
              key={recipe.id}
              className="bg-white rounded-xl border border-[#e1e3e4] overflow-hidden shadow-2xs hover:shadow-xs transition-all flex flex-col justify-between group"
            >
              {/* Image & Category Pills (clickable to view recipe details) */}
              <div
                onClick={() => setSelectedDetailRecipe(recipe)}
                className="relative h-20 sm:h-24 md:h-28 w-full overflow-hidden bg-[#e1e3e4] cursor-pointer"
                title="Haz clic para ver detalles de la receta"
              >
                <img
                  src={recipe.imageUrl}
                ...
                <div className="absolute top-1.5 left-1.5 flex flex-wrap gap-1 max-w-[80%]">
                  {getRecipeCategories(recipe).slice(0, 2).map((cat) => (
                    <span
                      key={cat}
                      className="bg-[#0f5238]/90 backdrop-blur-xs text-white text-[9px] sm:text-[10px] font-semibold px-2 py-0.5 rounded-full shadow-xs"
                    >
                      {cat}
                    </span>
                  ))}
                  {getRecipeCategories(recipe).length > 2 && (
                    <span className="bg-[#0f5238]/90 backdrop-blur-xs text-white text-[8px] sm:text-[9px] font-bold px-1.5 py-0.5 rounded-full shadow-xs">
                      +{getRecipeCategories(recipe).length - 2}
                    </span>
                  )}
                </div>
                {recipe.isAiGenerated && (
                  <span className="absolute top-1.5 right-1.5 bg-[#fc8a40] text-white text-[8px] sm:text-[9px] font-bold px-1.5 py-0.5 rounded-full flex items-center gap-0.5 shadow-xs">
                    <Sparkles className="w-2.5 h-2.5" />
                    IA
                  </span>
                )}
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/15 transition-colors flex items-center justify-center">
                  <span className="opacity-0 group-hover:opacity-100 transition-opacity bg-black/60 text-white text-[10px] font-semibold px-2 py-0.5 rounded-full flex items-center gap-1">
              ...
              <div className="p-2.5 sm:p-3 flex-1 flex flex-col justify-between space-y-1.5">
                ...
                >
                  <h3 className="font-extrabold text-sm sm:text-base text-[#191c1d] group-hover/title:text-[#0f5238] leading-tight line-clamp-1 font-heading transition-colors tracking-tight">
                    {recipe.name}
                  </h3>
                  {recipe.description && (
                    <p className="text-[11px] text-[#707973] line-clamp-1 mt-0.5">
                      {recipe.description}
                    </p>
                  )}
                </div>
                ...
                {/* Metrics */}
                <div className="flex items-center justify-between pt-1.5 border-t border-[#e1e3e4]/60 text-[11px] text-[#707973]">
                  <div className="flex items-center gap-2">
                    <span className="flex items-center gap-1 font-medium">
                      <Timer className="w-3 h-3 text-[#0f5238]" />
                      {recipe.timeMinutes}m
                    </span>
                    <span>•</span>
                    <span className="flex items-center gap-1 font-medium text-[#9b4500]">
                      <Flame className="w-3 h-3 text-[#9b4500]" />
                      {recipe.calories} kcal
                    </span>
                  </div>
                  <span className="text-[10px] font-semibold text-[#404943] bg-[#f3f4f5] px-1.5 py-0.5 rounded">
                    {recipe.difficulty}
                  </span>
                </div>
              </div>
              ...
              {/* Card Action */}
              <div className="p-2.5 sm:p-3 pt-0 flex items-center gap-1.5">
                <button
                  type="button"
                  onClick={() => handleOpenAssignModal(recipe)}
                  className="flex-1 bg-[#f8f9fa] hover:bg-[#0f5238] text-[#0f5238] hover:text-white border border-[#bfc9c1] hover:border-[#0f5238] font-semibold text-xs py-1.5 px-2 rounded-lg transition-all duration-200 flex items-center justify-center gap-1 cursor-pointer shadow-2xs active:scale-98"
                >
                  <Plus className="w-3.5 h-3.5 stroke-[2.5]" />
                  <span>Añadir</span>
                </button>
                ...
                {onEditRecipe && (
                  <button
                    ...
                    title="Editar receta"
                    className="p-1.5 text-[#0f5238] hover:bg-[#b1f0ce]/40 border border-[#bfc9c1] hover:border-[#0f5238] rounded-lg text-xs font-semibold flex items-center transition-all cursor-pointer shrink-0"
                  >
                    <Pencil className="w-3.5 h-3.5" />
                  </button>
                )}
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-[#e1e3e4] space-y-5 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div>
              ...
              <p className="text-[11px] text-[#707973] mt-1.5">
                {selectedDays.length === 0 ? (
                  <span className="text-[#0f5238] font-semibold">👉 Toca el día (o días) en que deseas planificar este plato</span>
                ) : (
                  <span>Has seleccionado {selectedDays.length} día{selectedDays.length > 1 ? 's' : ''} de la semana.</span>
                )}
              </p>
            </div>
            ...
            <div className="pt-3 border-t border-[#e1e3e4] flex items-center justify-end gap-3">
              ...
              <button
                ...
                disabled={feedbackSuccess || selectedDays.length === 0}
                className={`px-5 py-2.5 rounded-xl text-xs font-semibold text-white shadow-xs transition-all flex items-center gap-1.5 ${
                  feedbackSuccess
                    ? 'bg-[#0f5238] cursor-default'
                    : selectedDays.length === 0
                    ? 'bg-[#bfc9c1] text-[#707973] cursor-not-allowed opacity-80'
                    : 'bg-[#0f5238] hover:bg-[#2d6a4f] cursor-pointer active:scale-98'
                }`}
              >
                {feedbackSuccess ? (
                  <>
      ...
      {selectedDetailRecipe && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col shadow-xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="relative h-36 sm:h-48 md:h-56 w-full bg-[#f3f4f5] shrink-0">
              <img
              ...
              <div className="absolute bottom-3 left-3 flex flex-wrap items-center gap-1.5 max-w-[85%]">
                {getRecipeCategories(selectedDetailRecipe).map((cat) => (
                  <span
                    key={cat}
                    className="bg-[#0f5238] text-white text-xs font-semibold px-3 py-1 rounded-full shadow-xs"
                  >
                    {cat}
                  </span>
                ))}
            ...
            <div className="p-4 sm:p-6 overflow-y-auto space-y-4 sm:space-y-5 flex-1 custom-scrollbar">
              <div>
                <h2 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-[#191c1d] font-heading tracking-tight leading-tight">
